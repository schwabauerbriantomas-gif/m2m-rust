...
pub fn cmd_ingest(config: M2MConfig, input: PathBuf, n_clusters: Option<usize>, seed: u64) {
    ...
    eprintln!(
        "[m2m] Ingesting {} vectors ({}D) via DatasetTransformer",
        vectors.nrows(),
        vectors.ncols()
    );
...
pub fn cmd_ingest_hierarchical(
    ...
    eprintln!(
        "[m2m] Hierarchical ingest {} vectors ({}D)",
        vectors.nrows(),
        vectors.ncols()
    );
...
pub fn cmd_ingest_leader(
    ...
    eprintln!(
        "[m2m] Leader clustering ingest {} vectors ({}D)",
        vectors.nrows(),
        vectors.ncols()
    );
...
pub fn cmd_bench_gpu_ingest(n_vectors: usize, dim: usize, n_clusters: usize, n_queries: usize) {
    ...
    eprintln!(
        "[m2m] Phase 3: Search {} queries against {} splats...",
        n_queries, n_splats
    );
