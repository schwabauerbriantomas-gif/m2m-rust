...
pub fn make_persistence(
    ...
    backend: &str,
    rw: bool,
) -> Result<M2MPersistence, Box<dyn std::error::Error + Send + Sync>> {
...
pub fn load_or_create_store(
    data_dir: &str,
    config: &M2MConfig,
) -> (SplatStore, Option<M2MPersistence>) {
    ...
    if let Some(ref p) = persist {
        if let Ok(Some(vectors)) = p.load_vectors("default") {
            ...
            store.build_index();
            eprintln!(
                "[m2m] Loaded {} vectors from default shard",
                vectors.nrows()
            );
        }
    }
...
pub fn format_neighbors(
    results: &[m2m_vector_search::splats::NeighborResult],
    format: &str,
) -> String {
    match format {
        "json" => {
